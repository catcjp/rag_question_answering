...
# 硬编码测试（用你的新 Key）
api_key = "sk-ws-H.EHDMYIH.zyFm.MEYCIQCPwAUx8YX0LnS1AkXRq97IQ-dTkVdM8I9K_x2ACoAATgIhAKln4ywY8Svwg1gntcIihIrqqcTWpfHwQDeR3UGGCRcV"

# 方式二：如果环境变量没有，直接硬编码（测试用，记得改）
if not api_key:
    api_key = "sk-ws-H.EHDMPED.d1Xr.MEUCIQC1zhQ8zZ1Y4WC4r6zL6Y9Oy8l2hpf0v1mIV2C-0sm7ogIgQjgxLiEJqWD-m8Rdliq9whLyO8flcW63mclzx8SJYV4"  # 替换成你自己的 Key

if not api_key:
    st.error("❌ 未找到 API Key，请在 .env 文件中设置 DASHSCOPE_API_KEY 或直接在代码中填写")
    st.stop()
...
if uploaded_file is not None:
    ...
    with st.spinner("正在处理文档..."):
        ...
        # 分割文本（增大 chunk_size 和 overlap）
        text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=1000,  # 增大块大小，让章节内容更完整
            chunk_overlap=200,
            separators=["\n\n", "\n", "###", "一、", "二、", "三、", "。", "！", "？", "；", " ", ""]  # 加入章节标题分隔符
        )
        ...
        qa_chain = RetrievalQA.from_chain_type(
            ...
            chain_type="stuff",
            retriever=retriever,
            return_source_documents=True  # 加这一行
        )
        # 然后替换 Prompt
    ...
    with st.expander("🔍 调试：查看检索到的文本块"):
        if uploaded_file and 'retriever' in locals():
            sample_q = "实验目的 实验要求"
...
# 可选：添加调试信息（如果你想知道检索到了什么，取消下面注释）
# with st.expander("🔍 调试：查看检索到的文本块"):
#     if uploaded_file and 'retriever' in locals():
#         for i, doc in enumerate(retriever.get_relevant_documents("总结")):
#             st.write(f"--- 块 {i+1} ---")
